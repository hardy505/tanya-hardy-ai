...
# Tampilan awal jika belum ada riwayat chat
...
if prompt:
    if not api_key:
        ...
        # Batasi riwayat chat ke 4 pesan terakhir agar hemat token
            ...
            try:
                try:
                    completion = client.chat.completions.create(
                        ...
                        max_tokens=800,  # Menjaga batas OTPM agar tidak kena error 429
                        stream=True
                    )
                except Exception:
                    completion = client.chat.completions.create(
                ...
                response_container.empty()
                if "429" in str(e):
                    st.warning("⏳ Server sedang sibuk karena batas permintaan per menit. Silakan tunggu beberapa detik lalu coba lagi.")
                else:
